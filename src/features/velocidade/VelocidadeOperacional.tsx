...
export function VelocidadeOperacional() {
  ...
  const mAnt = useMemo(
    () => ({
      ...
      primContPag: avg(
        anteriores.map((j) => j.tAtendimentoMin + j.tConfirmacaoMin + j.tPagamentoMin),
      ),
    }),
    [anteriores],
  );
  ...
  const eficienciaHora = useMemo(() => {
    const buckets: Record<number, number[]> = {};
    atuais.forEach((j) =>
      (buckets[j.hora] ||= []).push(j.tAtendimentoMin + j.tConfirmacaoMin + j.tPagamentoMin),
    );
  ...
  const porAtendente = useMemo(
    () =>
      unique(atuais.map((j) => j.atendente))
        .map((a) => {
  ...
  const porPagamento = useMemo(
    () =>
      unique(atuais.map((j) => j.pagamento))
        .map((p) => {
          const list = atuais.filter((j) => j.pagamento === p);
          return {
            pag: p,
            ate_pag: avg(list.map((j) => j.tAtendimentoMin + j.tConfirmacaoMin + j.tPagamentoMin)),
          };
        })
        .sort((a, b) => a.ate_pag - b.ate_pag),
    [atuais],
  );
  ...
  const alertas = useMemo(() => {
    ...
    if (lentos > 0)
      list.push({
        tipo: "destructive",
        titulo: `${lentos} pedidos acima da média`,
        desc: `Jornada total > ${fmtMin(m.total * 1.5)} (média ${fmtMin(m.total)}).`,
      });
    if (m.wpp > 8)
      list.push({
        tipo: "warning",
        titulo: "Atendimento lento no WhatsApp",
        desc: `Resposta média ${fmtMin(m.wpp)} — ideal abaixo de 8min.`,
      });
    const atrasos = atuais.filter((j) => j.tEntregaMin > 90).length;
    if (atrasos > 0)
      list.push({
        tipo: "destructive",
        titulo: `${atrasos} entregas atrasadas`,
        desc: `Tempo de entrega > 1h30min.`,
      });
    const semConf = atuais.filter((j) => j.tConfirmacaoMin > 30).length;
    if (semConf > 0)
      list.push({
        tipo: "warning",
        titulo: `${semConf} clientes demorando para confirmar`,
        desc: `Confirmação acima de 30min — risco de abandono.`,
      });
    if (m.total < mAnt.total && mAnt.total > 0)
      list.push({
        tipo: "primary",
        titulo: "Operação ficou mais rápida",
        desc: `Jornada total reduziu ${Math.abs(deltaPct(m.total, mAnt.total)).toFixed(0)}% vs período anterior.`,
      });
    return list;
  }, [atuais, m, mAnt]);
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-end justify-between flex-wrap gap-3 px-1">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground/70 mt-0.5">
            métricas inteligentes em tempo real · jornada do cliente ponta a ponta
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
        <TempoKpi
          icon={<Bot />}
          label="Resposta da IA no WhatsApp"
          value={fmtMin(m.wpp)}
          delta={deltaPct(m.wpp, mAnt.wpp)}
          invert
        />
        <TempoKpi
          icon={<CheckCircle2 />}
          label="Conversa → fechamento"
          value={fmtMin(m.atendimento + m.confirmacao)}
        />
        <TempoKpi
          icon={<Zap />}
          label="Conversa → pagamento"
          value={fmtMin(m.primContPag)}
          delta={deltaPct(m.primContPag, mAnt.primContPag)}
          invert
        />
        <TempoKpi
          icon={<Truck />}
          label="Pagamento → entrega"
          value={fmtMin(m.pedEntrega)}
          delta={deltaPct(m.entrega, mAnt.entrega)}
          invert
        />
        <TempoKpi
          icon={<Route />}
          label="Jornada total do pedido"
          value={fmtMin(m.total)}
          delta={deltaPct(m.total, mAnt.total)}
          invert
          highlight
        />
      </div>
      ...
      <div className="card-soft p-3 flex items-center gap-3 text-xs">
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            início da conversa · fechamento da venda · pagamento · saída para entrega · entrega
            concluída — atualizado em tempo real
          </p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="card-soft p-5 lg:col-span-2">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Atendimento → Entregue · {atuais.length} pedidos no período
          </p>
              ...
              return (
                <div key={f.etapa}>
                  ...
                  <div className="h-6 rounded-lg bg-secondary overflow-hidden">
                    <div
                      className="h-full rounded-lg bg-gradient-to-r from-primary to-accent transition-all"
                      style={{ width: `${w}%` }}
                    />
                  </div>
                </div>
        ...
        <div className="card-soft p-5">
          <h3 className="font-semibold inline-flex items-center gap-2">
            <AlertTriangle className="size-4 text-accent" /> Alertas inteligentes
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">gargalos detectados pela IA</p>
          <ul className="mt-3 space-y-2">
            {alertas.length === 0 && (
              <li className="text-xs text-muted-foreground p-3 rounded-lg bg-secondary/40">
                Operação saudável ✅
              </li>
            )}
            {alertas.map((a, i) => {
              ...
              <BarChart data={eficienciaHora}>
                ...
                <Bar dataKey="min" radius={[6, 6, 0, 0]}>
                  {eficienciaHora.map((e, i) => (
                    <Cell
                      key={i}
                      fill={e.min === 0 ? "var(--color-secondary)" : "var(--color-accent)"}
                    />
                  ))}
                </Bar>
              </BarChart>
      ...
      <div className="card-soft p-5">
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3 mt-3">
          <Insight
            label="Horário que mais converte rápido"
            value={horarioConverte}
            hint="menor tempo 1º contato → pago"
          />
          <Insight
            label="Produto que fecha mais rápido"
            value={produtoRapido?.p ?? "—"}
            hint={produtoRapido ? `média ${fmtMin(produtoRapido.t)}` : ""}
          />
          <Insight
            label="Melhor atendente"
            value={porAtendente[0]?.atendente ?? "—"}
            hint={porAtendente[0] ? `jornada ${fmtMin(porAtendente[0].total)}` : ""}
          />
          <Insight
            label="Pagamento mais ágil"
            value={porPagamento[0]?.pag ?? "—"}
            hint={porPagamento[0] ? `até pago ${fmtMin(porPagamento[0].ate_pag)}` : ""}
          />
        </div>

        <div className="grid md:grid-cols-2 gap-4 mt-4">
          <MiniTable
            title="Tempo médio por região"
            rows={porRegiao.map((r) => ({ a: r.regiao, b: fmtMin(r.entrega) }))}
          />
          <MiniTable
            title="Performance por atendente"
            rows={porAtendente.map((a) => ({
              a: a.atendente,
              b: `${fmtMin(a.total)} · ${a.n} pedidos`,
            }))}
          />
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <RankCard
          ...
          rows={maisRapidos.map((j) => ({ a: j.cliente, b: fmtMin(jornadaTotal(j)) }))}
          tone="success"
        />
        <RankCard
          icon={<TrendingDown className="text-destructive" />}
          title="Pedidos mais demorados"
          rows={maisLentos.map((j) => ({ a: j.cliente, b: fmtMin(jornadaTotal(j)) }))}
          tone="destructive"
        />
        <RankCard
          icon={<Truck className="text-primary" />}
          title="Entregas mais rápidas"
          rows={entregasTop.map((j) => ({
            a: `${j.cliente} · ${j.regiao}`,
            b: fmtMin(j.tEntregaMin),
          }))}
          tone="primary"
        />
      </div>
  ...
  return (
    <div className={`card-soft p-4 ${highlight ? "ring-1 ring-primary/40" : ""}`}>
      <div className="flex items-center justify-between">
        <div className="size-8 rounded-lg grid place-items-center bg-primary/10 text-primary [&_svg]:size-4">
          {icon}
        </div>
        {showDelta && Trend && (
...
function RankCard({
  ...
  return (
    <div className="card-soft p-5">
      <h3 className="font-semibold inline-flex items-center gap-2 [&_svg]:size-4">
        {icon} {title}
      </h3>
      <ol className="mt-3 space-y-1.5">
        {rows.map((r, i) => (
          <li
            key={i}
            className="flex items-center gap-3 text-xs p-2 rounded-lg hover:bg-secondary/40"
          >
            <span
              className={`size-6 rounded-md grid place-items-center font-bold text-[10px] ${bg}`}
            >
              {i + 1}
            </span>
