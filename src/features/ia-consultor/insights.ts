...
export function gerarInsights(leads: LeadCard[] = []): Insight[] {
  ...
  if (leads.length > 0 && ticketAtual < 200) {
    insights.push({
      id: "ins-ticket",
      tipo: "ticket",
...
export function responderPergunta(pergunta: string, leads: LeadCard[] = []): string {
  ...
  if (q.includes("cupom") || q.includes("cupons")) {
    return "Ainda nao ha dados reais de cupons conectados para analise.";
  }
  if (q.includes("verm?fug") || q.includes("vermifug")) {
    return "Ainda nao ha dados reais de protecao antiparasitaria conectados.";
  }
  if (q.includes("upsell")) {
    return "Ainda nao ha clientes reais suficientes para sugerir upsell.";
  }
