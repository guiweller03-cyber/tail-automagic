...
const VERM_DEFAULT: VermifugoCfg = {
  ...
  msgLembrete:
    "Olá {nome}, notei que a proteção do {pet} venceu há {dias} dias. Posso te enviar um cupom?",
};
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Fluxos n8n + WhatsApp + IA executando em segundo plano
        </p>
      </div>
      ...
      <div className="inline-flex rounded-lg bg-muted p-1">
        {(
          [
            ["fluxos", "Fluxos gerais"],
            ["vermifugo", "Fluxo Vermífugo"],
            ["posvenda", "Pós-venda ração"],
          ] as const
        ).map(([k, l]) => (
          <button
            key={k}
            onClick={() => setAba(k)}
            className={`px-3 h-8 rounded-md text-xs font-semibold ${aba === k ? "bg-background shadow text-foreground" : "text-muted-foreground"}`}
          >
            {l}
          </button>
        ))}
          ...
          {items.map((a, i) => (
            <div key={a.nome} className="card-soft p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  <span
                    className={`absolute top-0.5 size-5 rounded-full bg-white shadow transition ${a.ativo ? "left-5" : "left-0.5"}`}
                  />
                </button>
              </div>
              ...
              <div className="flex gap-2 mt-3">
                ...
                <button className="flex-1 h-9 rounded-lg bg-secondary text-xs font-semibold hover:bg-secondary/70">
                  Editar fluxo
                </button>
              </div>
            </div>
...
function FluxoVermifugo() {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(VERM_KEY, JSON.stringify(cfg));
    } catch {
      /* noop */
    }
  }, [cfg]);
  ...
  function enviarAgora(p: PetEmProtecao) {
    const msg = (p.status === "vencido" ? cfg.msgLembrete : cfg.msgAviso)
      .replace("{nome}", p.cliente)
      .replace("{pet}", p.pet)
      .replace("{dias}", String(Math.abs(p.venceEm)));
    ...
    <div className="grid lg:grid-cols-[1fr_400px] gap-5">
      <div className="card-soft p-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Lembrete automático antes/depois da proteção vencer
              </p>
            </div>
          </div>
          <button
            onClick={() => setAtivo((v) => !v)}
            className={`relative w-12 h-6 rounded-full ${ativo ? "bg-success" : "bg-border"}`}
          >
            <span
              className={`absolute top-0.5 size-5 rounded-full bg-white shadow transition ${ativo ? "left-6" : "left-0.5"}`}
            />
          </button>
        </div>
        ...
        <div className="mt-5 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <NumField
              label="Duração proteção (d)"
              value={cfg.duracaoDias}
              onChange={(v) => setCfg({ ...cfg, duracaoDias: v })}
            />
            <NumField
              label="Aviso prévio (D-)"
              value={cfg.avisoPrevio}
              onChange={(v) => setCfg({ ...cfg, avisoPrevio: v })}
            />
            <NumField
              label="Lembrete após (D+)"
              value={cfg.lembreteDias}
              onChange={(v) => setCfg({ ...cfg, lembreteDias: v })}
            />
            <NumField
              label="Cupom após (D+)"
              value={cfg.cupomDias}
              onChange={(v) => setCfg({ ...cfg, cupomDias: v })}
            />
          </div>
          <Field label="Cupom automático">
            <input
              value={cfg.cupomCodigo}
              onChange={(e) => setCfg({ ...cfg, cupomCodigo: e.target.value.toUpperCase() })}
              className="cfg-input"
            />
          </Field>
          <Field label="Mensagem aviso prévio (use {nome} {pet} {dias})">
            <textarea
              rows={2}
              value={cfg.msgAviso}
              onChange={(e) => setCfg({ ...cfg, msgAviso: e.target.value })}
              className="cfg-input"
            />
          </Field>
          <Field label="Mensagem lembrete pós-vencimento">
            <textarea
              rows={2}
              value={cfg.msgLembrete}
              onChange={(e) => setCfg({ ...cfg, msgLembrete: e.target.value })}
              className="cfg-input"
            />
          </Field>
          <button
            onClick={() => toast.success("Configuração salva")}
            className="h-9 px-4 rounded-lg bg-foreground text-background text-sm font-semibold"
          >
            Salvar fluxo
          </button>
        </div>
      ...
      <div className="card-soft p-5">
        <h3 className="font-bold flex items-center gap-2">
          <Shield className="size-4 text-primary" /> Pets em proteção
        </h3>
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground truncate">
                      {p.produto} ·{" "}
                      {p.venceEm > 0
                        ? `vence em ${p.venceEm}d`
                        : `venceu há ${Math.abs(p.venceEm)}d`}
                    </div>
                  </div>
                  <button
                    onClick={() => enviarAgora(p)}
                    className="h-8 px-2.5 rounded-lg bg-success text-success-foreground text-[11px] font-semibold inline-flex items-center gap-1 hover:opacity-90"
                  >
...
function PosVendaConfig() {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(POSVENDA_KEY, JSON.stringify(cfgs));
    } catch {
      /* noop */
    }
  }, [cfgs]);
      ...
      arr.map((c, idx) =>
        idx === i
          ? {
              ...
              perguntas: [
                ...c.perguntas,
                { id: `q${Date.now()}`, texto: "Nova pergunta", aposDias: 5 },
              ],
            }
          : c,
      ),
  ...
  function removerPergunta(i: number, qid: string) {
    setCfgs((arr) =>
      arr.map((c, idx) =>
        idx === i ? { ...c, perguntas: c.perguntas.filter((q) => q.id !== qid) } : c,
      ),
    );
  }
    ...
    <div className="space-y-4">
      <div className="card-soft p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Perguntas automáticas configuráveis por marca/categoria
          </p>
        </div>
        <button
          onClick={addCfg}
          ...
          <div key={i} className="card-soft p-4 space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(e) =>
                  setCfgs((arr) =>
                    arr.map((x, idx) => (idx === i ? { ...x, marca: e.target.value } : x)),
                  )
                }
                className="cfg-input2"
                placeholder="Marca"
              />
              <input
                value={c.categoria}
                onChange={(e) =>
                  setCfgs((arr) =>
                    arr.map((x, idx) => (idx === i ? { ...x, categoria: e.target.value } : x)),
                  )
                }
                className="cfg-input2"
            ...
            <div className="space-y-2">
              {c.perguntas.map((q) => (
                <div key={q.id} className="flex items-center gap-2">
                  ...
                  <input
                    value={q.texto}
                    onChange={(e) => updatePergunta(i, q.id, { texto: e.target.value })}
                    className="cfg-input2 flex-1"
                  />
                  <input
                    type="number"
                    value={q.aposDias}
                    onChange={(e) =>
                      updatePergunta(i, q.id, { aposDias: Number(e.target.value) || 0 })
                    }
                    className="cfg-input2 w-16"
                  />
                  <button
                    onClick={() => removerPergunta(i, q.id)}
            ...
            <div className="pt-3 border-t border-border flex items-center justify-between text-xs">
              <span className="text-muted-foreground">
                <MessageCircle className="size-3 inline" /> {c.perguntas.length} mensagens por
                compra
              </span>
              <button
                onClick={() => toast.success("Regra salva")}
