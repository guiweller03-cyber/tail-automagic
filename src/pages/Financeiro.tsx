...
// Lançamentos manuais agora persistem no Supabase (antes ficavam só no
// localStorage e "sumiam" ao trocar de navegador/dispositivo).
const EP_FUEL = "/api/crm/financeiro/abastecimentos";
const EP_EXPENSES = "/api/crm/financeiro/despesas";
const EP_MARKETING = "/api/crm/financeiro/marketing";

function useFinanceiroList<T extends { id: string }>(endpoint: string) {
  ...
  useEffect(() => {
    ...
    fetch(endpoint, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
        if (alive && Array.isArray(data)) setLista(data as T[]);
      })
      .catch(() => {})
      .finally(() => {
  ...
  async function criar(input: Record<string, unknown>): Promise<boolean> {
    try {
  ...
  async function atualizar(id: string, patch: Record<string, unknown>): Promise<void> {
    ...
    setLista((prev) => prev.map((it) => (it.id === id ? ({ ...it, ...patch } as T) : it)));
    try {
      const res = await fetch(endpoint, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
      ...
      const atualizado = (await res.json()) as T;
      setLista((prev) => prev.map((it) => (it.id === id ? atualizado : it)));
    } catch (e) {
      setLista(anterior);
  ...
  return { lista, setLista, carregando, criar, atualizar, remover } as const;
}
  ...
  return (
    <div className="space-y-5">
      ...
      <Tabs defaultValue="vendas" className="space-y-5">
        <TabsList className="h-11 p-1 w-full">
          <TabsTrigger value="vendas" className="flex-1 px-2 sm:px-4 h-9">
            Vendas
          </TabsTrigger>
          <TabsTrigger value="combustivel" className="flex-1 px-2 sm:px-4 h-9">
            Combustível
          </TabsTrigger>
          <TabsTrigger value="despesas" className="flex-1 px-2 sm:px-4 h-9">
            Despesas
          </TabsTrigger>
          <TabsTrigger value="marketing" className="flex-1 px-2 sm:px-4 h-9">
            Marketing
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="combustivel" className="space-y-5">
          <AbaCombustivel />
        </TabsContent>
...
function AbaVendas() {
  ...
  const projTone =
    projecao >= meta ? "text-success" : pctProj >= 80 ? "text-accent" : "text-destructive";
    ...
    <>
          ...
          <ul className="text-sm space-y-2.5">
            ...
            <li className="pt-2 border-t border-border flex justify-between text-xs text-muted-foreground">
              ...
              <span className="font-bold text-success">
                {faturamentoMes > 0
                  ? `${((kpis.lucroMes / faturamentoMes) * 100).toFixed(1)}%`
                  : "0,0%"}
              </span>
            </li>
          </ul>
      ...
      <div className="card-soft p-5">
        ...
        <div className="flex items-baseline gap-2 mb-3 flex-wrap">
          ...
          {editingMeta ? (
            ...
            <button
              ...
              className="text-sm font-semibold inline-flex items-center gap-1.5 hover:text-primary"
            >
        ...
        <p className="text-xs text-muted-foreground mt-2">
          ...
          {falta > 0 ? `faltam ${brl(falta)} para bater a meta` : "🎉 meta batida!"}
        </p>
...
function AbaCombustivel() {
  const { lista, criar } = useFinanceiroList<Abastecimento>(EP_FUEL);
  ...
  const kmTotal =
    ordenada.length > 1 ? ordenada[ordenada.length - 1].kmAtual - ordenada[0].kmAtual : 0;
  ...
  async function salvar() {
    if (!km || !litros || !valorLitro) {
      toast.error("Preencha KM, litros e valor por litro");
      return;
    }
      ...
      <div className="card-soft">
        ...
        >
          <span className="inline-flex items-center gap-2">
            <Plus className="size-4" /> Registrar abastecimento
          </span>
          ...
          <div className="px-4 pb-4 border-t border-border pt-4 space-y-3">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <Field label="Data">
                <input
                  ...
                  className="h-10 w-full px-3 rounded-lg bg-secondary outline-none focus:ring-2 ring-primary/30"
                />
              </Field>
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className="h-10 px-4 rounded-lg bg-secondary text-sm font-semibold"
              >
                Cancelar
              </button>
              <button
                onClick={() => void salvar()}
                className="h-10 px-5 rounded-lg bg-primary text-primary-foreground text-sm font-bold"
              >
                Salvar
              </button>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {linhas.map((l) => {
                const tone =
                  l.kml != null && l.kml === melhor
                    ? "bg-success/10"
                    : l.kml != null && l.kml === pior && melhor !== pior
                      ? "bg-destructive/10"
                      : "";
                return (
                  <tr key={l.id} className={`border-t border-border ${tone}`}>
                    ...
                    <td className="p-3 text-right tabular-nums">
                      {l.kmAtual.toLocaleString("pt-BR")}
                    </td>
                    <td className="p-3 text-right tabular-nums">
                      {l.kmPerc != null ? `${l.kmPerc} km` : "—"}
                    </td>
                    ...
                    <td className="p-3 text-right tabular-nums">
                      {l.kml != null ? l.kml.toFixed(2) : "—"}
                    </td>
                    <td className="p-3 text-right tabular-nums">
                      {l.cpkm != null ? brl(l.cpkm) : "—"}
                    </td>
...
function AbaDespesas() {
  const { lista, criar, atualizar } = useFinanceiroList<Despesa>(EP_EXPENSES);
  ...
  function togglePago(id: string) {
    const atual = lista.find((d) => d.id === id);
    if (!atual) return;
    void atualizar(id, { pago: !atual.pago });
  }
    ...
    <>
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Big
          ...
          delta={totalPendente > 0 ? "atenção" : "ok"}
          up={totalPendente === 0}
          icon={<AlertCircle className="size-4" />}
        />
        <Big
          label="Maior despesa"
          value={maiorEntry ? brl(maiorEntry[1]) : "—"}
          delta={maiorEntry ? CAT_LABEL[maiorEntry[0] as CategoriaDespesa] : "—"}
          up
        />
      </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="h-10 px-4 rounded-lg bg-primary text-primary-foreground text-sm font-bold inline-flex items-center gap-2"
        >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <button
                          ...
                          className={`h-7 px-2.5 rounded-md text-xs font-bold ${d.pago ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
                        >
                ...
                <li key={cat}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="inline-flex items-center gap-1.5">
                      <span>{CAT_EMOJI[cat as CategoriaDespesa]}</span>
                      {CAT_LABEL[cat as CategoriaDespesa]}
                    </span>
              ...
              >
                {Object.entries(CAT_LABEL).map(([k, v]) => (
                  <option key={k} value={k}>
                    {CAT_EMOJI[k as CategoriaDespesa]} {v}
                  </option>
                ))}
              </select>
            </Field>
...
function AbaMarketing() {
  const { lista, criar } = useFinanceiroList<GastoMkt>(EP_MARKETING);
  const [modal, setModal] = useState(false);
  const [data, setData] = useState(() => new Date().toISOString().slice(0, 10));
  ...
  return (
    <>
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Big
          label="Melhor ROI"
          value={melhor ? `${melhor.roi!.toFixed(1)}x` : "—"}
          delta={melhor ? TIPO_LABEL[melhor.tipo] : "—"}
      ...
      <div className="grid lg:grid-cols-3 gap-4">
                ...
                {lista.map((g) => {
                  const roiBadge =
                    g.roi == null ? (
                      ...
                      <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-success/20 text-success">
                        Excelente · {g.roi.toFixed(1)}x
                      </span>
                    ) : g.roi < 1 ? (
                      <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-destructive/20 text-destructive">
                        Prejuízo · {g.roi.toFixed(1)}x
                      </span>
                    ) : (
                  ...
                  return (
                    <tr key={g.id} className="border-t border-border">
                      ...
                      <td className="p-3">
                        <span
                          className={`px-2 py-0.5 rounded text-[11px] font-bold ${TIPO_TONE[g.tipo]}`}
                        >
                          {TIPO_LABEL[g.tipo]}
                        </span>
                      </td>
                      ...
                      <td className="p-3 text-center">
                        <span
                          className={`px-2 py-0.5 rounded text-[11px] font-bold ${g.pago ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
                        >
        ...
        <div className="card-soft p-5">
          <h3 className="font-semibold mb-3 inline-flex items-center gap-2">
            <Trophy className="size-4" /> Ranking por ROI
          </h3>
          <ul className="space-y-3">
            ...
            {ranking.length === 0 && (
              <li className="text-sm text-muted-foreground">Sem ROI registrado ainda</li>
            )}
          </ul>
        </div>
      </div>
        ...
        <Modal onClose={() => setModal(false)} title="Novo gasto de marketing">
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <Field label="Tipo">
              <select
                value={tipo}
                onChange={(e) => setTipo(e.target.value as TipoMkt)}
                className="h-10 w-full px-3 rounded-lg bg-secondary outline-none focus:ring-2 ring-primary/30"
              >
            ...
            <Field label="Resultado (opcional)" full>
              <input
                value={resultado}
                onChange={(e) => setResultado(e.target.value)}
                placeholder="42 leads · 11 vendas"
                className="h-10 w-full px-3 rounded-lg bg-secondary outline-none focus:ring-2 ring-primary/30"
              />
            </Field>
            <Field label="ROI (opcional)">
              <input
                type="number"
                step="0.1"
                value={roi}
...
function Line({
  ...
  return (
    <li
      className={`flex justify-between ${bold ? "font-bold pt-2 border-t border-border" : ""} ${muted ? "text-muted-foreground" : ""} ${accent ? "text-success font-semibold" : ""}`}
    >
      <span>{label}</span>
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4 bg-black/60" onClick={onClose}>
      <div
        className="card-soft bg-background w-full max-w-lg p-5"
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={onClose}
            className="size-8 rounded-lg grid place-items-center hover:bg-secondary"
          >
            <X className="size-4" />
          </button>
        </div>
