...
export function PDV({ initialCliente = "", initialTelefone = "" }: PDVProps) {
  ...
  const finalizar = async (whats = false) => {
    ...
    if (modoAvulso && carrinho.length === 0 && !obs.trim()) {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0,00"
                    className="w-full h-12 pl-12 pr-4 rounded-xl bg-secondary text-2xl font-bold outline-none focus:ring-2 ring-primary/30 tabular-nums"
                  />
                </div>
              </div>
            </div>
