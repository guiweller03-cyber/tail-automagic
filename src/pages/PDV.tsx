...
export function PDV() {
  ...
  useEffect(() => {
    async function carregarPdv() {
      try {
        ...
        if (produtosResponse.ok) setProdutos((await produtosResponse.json()) as Produto[]);
        if (clientesResponse.ok) setClientes((await clientesResponse.json()) as Cliente[]);
      } catch (error) {
  ...
  const totalFull = Math.max(0, subtotalFull + frete - desconto - cupomDesconto);
  const lucro = totalFull - custoFull;
  const margem = totalFull > 0 ? (lucro / totalFull) * 100 : 0;
  const totalRapido = Number(valor.replace(",", ".")) || 0;
  const totalRapidoFinal = Math.max(0, totalRapido - cupomDesconto);
  const total = expandido ? totalFull : totalRapidoFinal;
  ...
  const validarCupom = async () => {
    ...
    const base = expandido ? Math.max(0, subtotalFull + frete - desconto) : totalRapido;
    if (!codigo || base <= 0) return;
    ...
    try {
      const response = await fetch("/api/crm/indicacoes", {
        ...
        body: JSON.stringify({ acao: "validar_cupom", codigo, total: base }),
      });
      const data = (await response.json()) as {
        valido?: boolean;
        cupom?: { desconto?: number; cupom?: { codigo?: string } };
        erro?: string;
      };
      if (!response.ok || !data.valido || !data.cupom) {
        setCupomDesconto(0);
        setCupomErro(data.erro ?? "Cupom invalido ou indisponivel.");
        return;
      }

      setCupomCodigo(data.cupom.cupom?.codigo ?? codigo.toUpperCase());
      setCupomDesconto(Number(data.cupom.desconto ?? 0));
    } catch (error) {
      setCupomErro(error instanceof Error ? error.message : "Erro ao validar cupom.");
    }
  };
  ...
  const finalizar = async (whats = false) => {
    ...
    const c = clientes.find((x) => x.nome.toLowerCase() === nome.toLowerCase());
    if (!c?.telefone) {
      setErro("Selecione um cliente cadastrado com telefone para gravar a venda.");
      return;
    }

    const itens = expandido
      ? [...carrinho]
      : [
          {
            sku: "avulso",
            nome: obs || "Venda rápida",
            preco: totalRapido,
            precoCompra: 0,
            qtd: 1,
          },
    ...
    try {
      const response = await fetch("/api/crm/pedidos", {
        ...
        body: JSON.stringify({
          ...
          total,
          formaPagamento: pay,
          observacao: obs || (expandido ? "Venda PDV" : "Venda rapida PDV"),
          bairro: c.bairro,
          pet: c.pets[0],
          pago: statusPag === "Pago",
          itens: expandido
            ...
            : [],
          cupomCodigo: cupomDesconto > 0 ? cupomCodigo : null,
        }),
      });
      const pedido = (await response.json()) as { id?: string; hora?: string; erro?: string };
      if (!response.ok) throw new Error(pedido.erro ?? "Erro ao gravar venda no CRM");

      const v: Venda = {
        id: pedido.id ?? `PDV-${Date.now()}`,
        hora:
          pedido.hora ??
  ...
  const confirmarCancelamento = async () => {
    ...
    try {
      const response = await fetch("/api/crm/pedidos", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: cancelTarget.id, status: "cancelado" }),
      });
      const data = (await response.json()) as { erro?: string };
      if (!response.ok) throw new Error(data.erro ?? "Erro ao cancelar venda no CRM");
  ...
  const add = (p: Produto) =>
    setCarrinho((c) => {
      ...
      return [
        ...c,
        { sku: p.sku, nome: p.nome, preco: p.preco, precoCompra: p.precoCompra, qtd: 1 },
      ];
    });
  const change = (sku: string, delta: number) =>
    setCarrinho((c) =>
      c.flatMap((i) =>
        i.sku === sku ? (i.qtd + delta <= 0 ? [] : [{ ...i, qtd: i.qtd + delta }]) : [i],
      ),
    );
    ...
    <div className="space-y-4">
      ...
      {!expandido ? (
        <div className="card-soft p-5 max-w-2xl mx-auto space-y-4">
          ...
          <div>
            <Label icon={<Tag className="size-3.5" />}>Cupom</Label>
            <div className="grid grid-cols-[1fr_auto] gap-2">
              <input
                ...
                className="h-11 px-4 rounded-xl bg-secondary text-sm outline-none focus:ring-2 ring-primary/30"
              />
              <button
                onClick={() => void validarCupom()}
                disabled={!cupomCodigo.trim() || totalRapido <= 0}
                className="h-11 px-4 rounded-xl bg-card border border-border text-sm font-semibold hover:bg-secondary disabled:opacity-40"
              >
                Aplicar
              </button>
            </div>
            {cupomDesconto > 0 && (
              <div className="mt-1 text-[11px] text-success">Desconto aplicado: {brl(cupomDesconto)}</div>
            )}
            {cupomErro && <div className="mt-1 text-[11px] text-destructive">{cupomErro}</div>}
          </div>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2">
            <button
              ...
              disabled={!cliente || !totalRapido || salvando}
            ...
            <button
              onClick={() => (cliente && totalRapido ? void finalizar(true) : enviarWhats())}
            ...
            <div className="border-t border-border p-4 space-y-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-[1fr_auto] gap-2">
                  ...
                  <button
                    onClick={() => void validarCupom()}
                    disabled={!cupomCodigo.trim() || subtotalFull <= 0}
                    className="h-9 px-3 rounded-lg bg-card border border-border text-xs font-semibold hover:bg-secondary disabled:opacity-40"
                  >
                    Aplicar
                  </button>
                </div>
              ...
              <div className="rounded-xl bg-secondary/60 p-3 space-y-1.5">
                ...
                {cupomDesconto > 0 && <Row label={`Cupom ${cupomCodigo}`} value={`-${brl(cupomDesconto)}`} muted />}
              ...
              <div className="grid grid-cols-[1fr_auto] gap-2">
                ...
                <button
                  onClick={() => (carrinho.length ? void finalizar(true) : enviarWhats())}
                  disabled={!cliente || salvando}
        ...
        >
          ...
          >
            ...
            <div className="rounded-xl bg-warning/10 border border-warning/30 p-3 text-xs space-y-1">
              ...
              <div className="text-muted-foreground">↺ Produtos faturados retornam ao estoque</div>
              <div className="text-muted-foreground">↺ Totais do cliente sao estornados no CRM</div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!cancelMotivo.trim() || salvando}
