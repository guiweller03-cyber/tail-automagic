...
export function Entregas() {
  ...
  const receitaPrevista = listaEntregas
      ...
      prev.map((x) =>
        x.id === e.id
          ? {
              ...
              status: "concluída",
              historicoMensagens: [
                ...x.historicoMensagens,
                { hora: horaAgora(), texto: msg, enviado: true },
              ],
            }
          : x,
      ),
      ...
      prev.map((x) =>
        x.id === e.id
          ? {
              ...
              historicoMensagens: [
                ...x.historicoMensagens,
                { hora: horaAgora(), texto, enviado: true },
              ],
            }
          : x,
      ),
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {listaEntregas.length} paradas · {concluidas} concluídas
          </p>
        </div>
        <button
          onClick={iniciarRota}
        ...
        {!rotaConfigurada ? (
          <>
            <h3 className="font-semibold mb-3 inline-flex items-center gap-2">
              <RouteIcon className="size-4" /> Configurar rota do dia
            </h3>
            <div className="grid sm:grid-cols-2 gap-3 mb-3">
              <label className="block">
                ...
                <input
                  ...
                  placeholder="Loja — R. Principal, 100, Tijucas"
            ...
            <button
              ...
              className="h-10 px-5 rounded-lg bg-primary text-primary-foreground text-sm font-bold inline-flex items-center gap-2"
            >
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <button
              onClick={() => setRotaConfigurada(false)}
              className="h-9 px-3 rounded-lg bg-secondary text-xs font-semibold"
            >
              Reconfigurar
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card-soft p-0 overflow-hidden">
            <div className="relative h-72 bg-gradient-to-br from-primary/15 via-secondary to-accent/10">
              <div className="absolute inset-0 grid place-items-center text-muted-foreground pointer-events-none">
                <div className="text-center">
                  ...
                  <p className="text-xs">
                    {listaEntregas.length} paradas · ~{distanciaTotal.toFixed(0)} km · {tempoStr}
                  </p>
                </div>
              </div>
            ...
            <div className="px-4 py-3 border-t border-border flex flex-wrap gap-x-6 gap-y-1 text-xs text-muted-foreground">
              <span>
                <strong className="text-foreground">{distanciaTotal.toFixed(0)} km</strong>{" "}
                distância
              </span>
              <span>
                <strong className="text-foreground">{tempoStr}</strong> estimado
              </span>
              <span>
                <strong className="text-foreground">
                  {custoComb.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                </strong>{" "}
                combustível
              </span>
            </div>
            ...
            <div className="divide-y divide-border">
              {entregasOrdenadas.map((e) => {
                ...
                return (
                  <div
                    key={e.id}
                    className={`p-4 flex items-start gap-3 ${isConcluida ? "bg-success/5" : ""}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div
                        className={`text-xs text-muted-foreground flex items-center gap-1 mt-0.5 ${isConcluida ? "line-through" : ""}`}
                      >
                      ...
                      <div className="flex gap-2 mt-3 flex-wrap">
                        <button
                          ...
                          className="h-9 px-3 rounded-lg bg-success text-success-foreground text-xs font-semibold inline-flex items-center gap-1.5 disabled:opacity-50"
                        >
                          <CheckCircle2 className="size-3.5" />{" "}
                          {isConcluida ? "Entregue" : "Marcar entregue"}
                        </button>
                        <button
                          onClick={() => setAvisoAberto(e)}
        ...
        <div className="space-y-4">
          <div className="card-soft p-5">
            ...
            <ul className="space-y-2.5 text-sm">
              <li className="flex justify-between">
                ...
                <span className="font-semibold">{distanciaTotal.toFixed(0)} km</span>
              </li>
              <li className="flex justify-between">
                <span className="text-muted-foreground">Tempo estimado</span>
                <span className="font-semibold">{tempoStr}</span>
              </li>
              <li className="flex justify-between">
              ...
              <li className="flex justify-between">
                ...
                <span className="font-semibold text-success">
                  {receitaPrevista.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                </span>
              </li>
            </ul>
            <div className="mt-4">
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-semibold">
                  {concluidas} de {listaEntregas.length}
                </span>
              </div>
              <div className="h-2 bg-secondary rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${listaEntregas.length ? (concluidas / listaEntregas.length) * 100 : 0}%`,
                  }}
                />
              </div>
            </div>
          ...
          <div className="card-soft p-5 bg-success/5 border-success/30">
            <div className="flex items-center justify-between mb-2">
              ...
              >
                <span
                  className={`absolute top-0.5 size-4 bg-background rounded-full transition ${avisoAuto ? "left-5" : "left-0.5"}`}
                />
              </button>
            </div>
            <p className="text-sm">
      ...
      {avisoAberto && (
        ...
        >
          <div
            className="card-soft bg-background w-full max-w-md p-5"
          ...
          >
            ...
            <div className="space-y-2.5">
              {[
                {
                  titulo: "Saindo agora 🚗",
                  msg: `Olá ${primeiroNome(avisoAberto.cliente)}! Estou saindo agora para sua entrega. Chego em aproximadamente ${avisoAberto.eta} 🐾`,
                },
                {
                  titulo: "Chegando em 10 min ⏰",
                  msg: `Olá ${primeiroNome(avisoAberto.cliente)}! Estou a 10 minutinhos aí! Pode deixar alguém para receber? 🚗`,
                },
                {
                  titulo: "Problema na entrega ⚠️",
                  msg: `Olá ${primeiroNome(avisoAberto.cliente)}! Tivemos um imprevisto na entrega do seu pedido. Vou te ligar em breve para alinhar, ok?`,
                },
              ].map((opt) => (
                <div key={opt.titulo} className="rounded-xl border border-border p-3">
                  ...
                  <button
                    onClick={() => enviarAviso(avisoAberto, opt.msg)}
                    className="h-9 px-3 rounded-lg bg-success text-success-foreground text-xs font-bold inline-flex items-center gap-1.5"
                  >
      ...
      <aside className="fixed top-0 right-0 z-50 h-screen w-full sm:w-[400px] bg-background border-l border-border flex flex-col shadow-2xl">
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-start justify-between gap-2">
            ...
            <button
              onClick={onClose}
              className="size-8 rounded-lg grid place-items-center hover:bg-secondary shrink-0"
            >
              <X className="size-4" />
            </button>
          </div>
          <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              value={
                entrega.status === "concluída"
                  ...
                  : entrega.status === "em rota"
                    ? "em rota"
                    : entrega.status === "problema"
                      ? "cancelado"
                      : "pago"
              }
            />
            <a
              href={`tel:+55${entrega.telefone}`}
            ...
            <button
              onClick={() => waOpen(entrega.telefone, "")}
              className="h-8 px-3 rounded-lg bg-success text-success-foreground text-xs font-semibold inline-flex items-center gap-1.5"
            >
        ...
        <div className="p-4 border-b border-border space-y-2 text-sm">
          ...
          <div className="text-xs text-muted-foreground">
            ETA: <span className="font-semibold text-foreground">{entrega.eta}</span>
          </div>
          ...
          <button
            onClick={onNavegar}
            className="w-full h-9 rounded-lg bg-secondary text-xs font-semibold inline-flex items-center justify-center gap-1.5"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-secondary/30">
          ...
          {entrega.historicoMensagens.map((m, i) => (
            <div key={i} className={`flex ${m.enviado ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm ${m.enviado ? "bg-success text-success-foreground rounded-br-sm" : "bg-background rounded-bl-sm border border-border"}`}
              >
                <div>{m.texto}</div>
                <div
                  className={`text-[10px] mt-1 ${m.enviado ? "text-success-foreground/70" : "text-muted-foreground"} text-right`}
                >
                  {m.hora}
                </div>
              </div>
        ...
        <div className="p-3 border-t border-border space-y-2">
          <div className="flex flex-wrap gap-1.5">
            {rapidas.map((r) => (
              <button
                key={r}
                onClick={() => setTexto(r)}
                className="h-7 px-2.5 rounded-full bg-secondary text-[11px] font-medium hover:bg-accent hover:text-accent-foreground"
              >
                {r}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={enviar}
              className="h-10 px-4 rounded-lg bg-success text-success-foreground text-sm font-bold inline-flex items-center gap-1.5"
            >
